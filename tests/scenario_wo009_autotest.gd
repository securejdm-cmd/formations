...
func _initialize() -> void:
	...
	if compass_exit != 0:
		...
		print("[WO-009] Compass test PASS (32/32)")
...
func _run_fast_certification() -> void:
	...
	if _cert_realtime_trace != fast_trace:
		...
		print("[WO-009] Fast-mode certification PASS (seed %d trace byte-identical)" % CERT_SEED)
...
func _check_s1_regression(seed_value: int) -> void:
	...
	if not baseline.is_empty() and _core_trace(_scenario.get_trace_text()) != _core_trace(baseline):
		...
		_exit_code = 1
	print("[WO-009] S1 seed %d PASS winner=%s combat=%.1fs" % [seed_value, winner, phases.combat_sec])
...
func _check_s2_regression(seed_value: int) -> void:
	...
	if not baseline.is_empty() and _core_trace(_scenario.get_trace_text()) != _core_trace(baseline):
		...
		_exit_code = 1
	print("[WO-009] S2 seed %d PASS combat=%.1fs rout=%.2f" % [seed_value, phases.combat_sec, rout])
...
func _check_scenario_03() -> void:
	...
	if not baseline.is_empty() and _core_trace(_scenario.get_trace_text()) != _core_trace(baseline):
		...
		_exit_code = 1
	print("[WO-009] S3 PASS ratio=%.3f rout=%.2f" % [ratio, rout])
...
func _check_reflection_pair(seed_value: int) -> void:
	if _scenario.had_overlap_failure():
		...
		print("[WO-009] Overlap/adhesion seed %d PASS" % seed_value)
...
func _check_scenario_05() -> void:
	...
	if "blue_rally" not in trace or "rallying" not in trace:
		...
		_exit_code = 1
	print(
		"[WO-009] S5 PASS standard=%s rally=%s cohesion=%.1f"
...
func _check_scenario_06() -> void:
	...
	if _scenario.get_pursuit_tick_count() <= 0:
		...
		_exit_code = 1
	print(
		"[WO-009] S6 PASS rally=%s pursuit_ticks=%d strength=%.2f"
...
func _check_scenario_07() -> void:
	...
	for neighbor in _scenario.get_red_neighbors():
		...
		if neighbor.get_state() == Unit.State.WAVERING:
			tipped = true
	print("[WO-009] S7 PASS shocks=%d shock_tips_wavering=%s" % [shocks.size(), tipped])
...
func _check_scenario_08(single_damage: float) -> void:
	...
	print(
		"[WO-009] S8 single_damage=%.2f triple_damage=%.2f ratio=%.3f"
...
func _check_perf_40() -> void:
	print(
		"[WO-009] Perf40 min_fps=%.1f avg_fps=%.1f avg_tick_ms=%.3f p95_tick_ms=%.3f ticks=%d"
		% [
