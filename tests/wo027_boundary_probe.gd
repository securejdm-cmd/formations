extends SceneTree
## WO-027 Task 4 — R21 maneuver-dominance boundary (QoD ON at probe sigma).
...
func _run() -> void:
	# Probe at sigma=0.045 (0% cell in band; Task 2 did not select a width).
	_consts().set_constant("quality_of_day_enabled", true)
	_consts().set_constant("quality_of_day_sigma", 0.045)
	print("WO027_BOUNDARY sigma=0.045 enabled=true")
