extends SceneTree
## WO-030 Task 0 — S8 sublinear DIRECTION + variance diagnosis (n=500, QoD on σ=0.045).
## Args: START=N COUNT=N WORKER=id

const SIGMA := 0.045
const SEED_START_DEFAULT := 1000
const SEED_COUNT_DEFAULT := 500
const OUT_DIR := "res://docs/reports/evidence_wo030"
...
func _run() -> void:
	...
	print("WO030_S8_SWEEP start=%d count=%d sigma=%.3f worker=%s" % [start, count, SIGMA, worker])
	...
	print(
		"WO030_S8_STATS worker=%s n=%d mean=%.6f sd=%.6f cv=%.4f min=%.6f max=%.6f ge3=%d ms=%d"
		% [
			worker,
			n,
			mean,
	...
	print("WO030_S8_SWEEP_DONE worker=%s csv=%s" % [worker, csv_path])
