...
func _spawn_brace_charge_exhibit(origin: Vector2, px: float) -> void:
	...
	_add_caption_below_unit(spears, "Braced spears")

	var cav: Unit = UNIT_SCENE.instantiate()
	add_child(cav)
	cav.configure("charge_cav", "red", cav_profile, origin + Vector2(-50.0, 0.0), Vector2.RIGHT)
	...
	flash.position = origin - Vector2(14.0, 14.0)
