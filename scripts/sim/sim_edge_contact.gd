class_name SimEdgeContact
extends RefCounted

const _TickProfiler := preload("res://scripts/tick_profiler.gd")
const _SimContactCache := preload("res://scripts/sim/sim_contact_cache.gd")
...
static func begin_tick(tick_id: int) -> void:
	_SimContactCache.begin_tick(tick_id)
...
static func classify_contact(attacker: Variant, defender: Variant) -> Dictionary:
	...
	var cached = _SimContactCache.lookup(attacker, defender)
	if cached != null:
		result = cached
	else:
		result = _classify_contact_impl(attacker, defender)
		_SimContactCache.store(attacker, defender, result)
...
static func _classify_contact_impl(attacker: Variant, defender: Variant) -> Dictionary:
	...
	if (
		SimCombatResolver.is_head_on_pair(attacker, defender)
		and not SimCombatResolver.units_have_front_contact(attacker, defender)
	):
		return _empty_contact()

	var px_per_meter: Variant = Constants.get_float("px_per_meter")
	var forward: Vector2 = defender.facing.normalized()
	var left: Variant = SimFormationGeometry.left_vector(forward)
	var half_depth_m: float = defender.effective_depth_m() * 0.5
	var half_frontage_m: float = defender.effective_frontage_m() * 0.5

	var attacker_corners: Variant = SimFormationGeometry.get_corners(attacker)
	...
	var center_local: Variant = attacker.position - defender.position
	var center_along: Variant = center_local.dot(forward) / px_per_meter
	var center_across: Variant = center_local.dot(left) / px_per_meter
...
static func _attacker_front_face_contact_pct(attacker: Variant, defender: Variant, px_per_meter: float) -> float:
	...
	var right: Variant = SimFormationGeometry.right_vector(forward)
	var half_depth_m: float = attacker.effective_depth_m() * 0.5
	var half_frontage_m: float = attacker.effective_frontage_m() * 0.5

	var defender_corners: Variant = SimFormationGeometry.get_corners(defender)
	var along_min: Variant = INF
	var along_max: Variant = -INF
	var across_min: Variant = INF
	var across_max: Variant = -INF
