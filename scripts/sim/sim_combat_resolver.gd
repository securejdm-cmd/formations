...
static func contact_epsilon_m() -> float:
	return SimEdgeContact.contact_epsilon_m()
...
static func units_overlap(unit_a: Variant, unit_b: Variant) -> bool:
	...
	if (
		...
		return false
	if not SimFormationGeometry.bounds_may_overlap(unit_a, unit_b):
		return false
	if not could_have_contact(unit_a, unit_b):
		return false
	if unit_a.team_id == unit_b.team_id:
		return SimFormationGeometry.rectangles_overlap(unit_a, unit_b)
	if is_head_on_pair(unit_a, unit_b):
		return units_penetrating(unit_a, unit_b)
	if (
		SimEdgeContact.units_have_contact(unit_a, unit_b)
		or SimEdgeContact.units_have_contact(unit_b, unit_a)
	):
		return false
	return SimFormationGeometry.rectangles_overlap(unit_a, unit_b)
...
static func pair_has_classifier_contact(unit_a: Variant, unit_b: Variant) -> bool:
	if (
		...
		return false
	if is_head_on_pair(unit_a, unit_b) and not SimEdgeContact.has_non_front_segment_contact(unit_a, unit_b):
		return units_have_any_contact(unit_a, unit_b)
	var orient: Variant = SimEdgeContact.pick_segment_orientation(unit_a, unit_b)
...
static func _segment_adhesion_move_dirs(
	...
	for raw in [
		...
		if raw.length_squared() <= 0.0001:
			continue
		var key: Variant = "%.3f,%.3f" % [raw.x, raw.y]
		if seen.has(key):
			continue
		seen[key] = true
...
static func separate_allied_overlap(unit_a: Variant, unit_b: Variant) -> void:
	...
	if unit_a.team_id != unit_b.team_id:
		return
	if not SimFormationGeometry.rectangles_overlap(unit_a, unit_b):
		return
...
static func clamp_march_distance(unit: Variant, enemy: Variant, move_px: float) -> float:
	...
	if (
		SimEdgeContact.units_have_contact(unit, enemy)
		or SimEdgeContact.units_have_contact(enemy, unit)
	):
		return 0.0
...
static func _edge_shift_multiplier_for_name(edge_name: String) -> float:
	match edge_name:
		SimEdgeContact.EDGE_FRONT:
			return Constants.get_float("edge_mult_front")
		SimEdgeContact.EDGE_LEFT, SimEdgeContact.EDGE_RIGHT:
			return Constants.get_float("edge_mult_side_shift")
		SimEdgeContact.EDGE_REAR:
